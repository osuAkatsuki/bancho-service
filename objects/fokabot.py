...
def fokabotResponse(fro: str, chan: str, message: str) -> Union[str, bool]:
    """
    ...
    for rgx, cmd in fokabotCommands._commands:
        ...
        if (
            cmd['privileges'] and
            not userUtils.getPrivileges(userID) & cmd['privileges']
        ):
            return False

        # If this is an !mp command in a match, make sure the user is a referee.
        if (
            chan.startswith('#multi_') and
            cmd['trigger'] == '!mp' and
            not glob.matches.getMatchFromChannel(chan).is_referee(userID)
        ):
            return False
        ...
        if (
            cmd['syntax'] and
            len(message_split) <= cmd['syntax'].count(' ') + 1
        ):
            return {
        ...
        if cmd['callback']:
            ...
            thread = threading.Thread(
                target=lambda q, arg1, arg2, arg3: q.put(handle_command(cmd, arg1, arg2, arg3)),

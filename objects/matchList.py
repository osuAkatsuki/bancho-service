...
#         self.lastID: int = 1
    ...
    def disposeMatch(self, match_id: int) -> None:
        """
        ...
        for _slot in slots:
            _token = glob.tokens.getTokenFromUserID(_slot["user_id"], ignoreIRC=True)
    ...
    def getMatchByID(self, match_id: int) -> Optional[match.Match]:
        if self.matchExists(match_id):

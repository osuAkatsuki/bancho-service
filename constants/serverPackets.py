...
from constants import dataTypes, packetIDs, userRanks
...
loginFailed = packetHelper.buildPacket(packetIDs.server_userID, (
    (-1, dataTypes.SINT32),
))

loginBanned = (
    packetHelper.buildPacket(packetIDs.server_userID, (
        (-1, dataTypes.SINT32),
    )) +
    notification('You are banned. '
...
loginLocked = (
    ...
    notification("Your account is locked. You can't log in, but your "
                 "profile and scores are still visible from the website. "
                 "The earliest we accept appeals is 2 months after your "
                 "most recent offense, and really only care for the truth.")
)

forceUpdate = packetHelper.buildPacket(packetIDs.server_userID, (
    (-2, dataTypes.SINT32),
))

loginError = packetHelper.buildPacket(packetIDs.server_userID, (
    (-5, dataTypes.SINT32),
))
needSupporter = packetHelper.buildPacket(packetIDs.server_userID, (
    (-6, dataTypes.SINT32),
))
needVerification = packetHelper.buildPacket(packetIDs.server_userID, (
    (-8, dataTypes.SINT32),
))
...
def userSupporterGMT(supporter: bool, GMT: bool, tournamentStaff: bool) -> bytes:
    ...
    return packetHelper.buildPacket(packetIDs.server_supporterGMT, (
...
def onlineUsers() -> bytes:
    ...
    return packetHelper.buildPacket(packetIDs.server_userPresenceBundle, (
...
def userStats(userID: int, force: bool = False) -> bytes:
    ...
    return packetHelper.buildPacket(packetIDs.server_userStats, (
        ...
        (userToken.rankedScore if userToken.pp < 0x8000 else userToken.pp, dataTypes.UINT64),
...
def targetBlockingDMs(to: str, fro: str, fro_id: int = 0) -> bytes:
    return packetHelper.buildPacket(packetIDs.server_targetBlockingNonFriendsDM, (
        (fro, dataTypes.STRING),
        ('', dataTypes.STRING),
        (to, dataTypes.STRING),
        (fro_id or userUtils.getID(fro), dataTypes.SINT32)
...
def targetSilenced(to: str, fro: str, fro_id: int = 0) -> bytes:
    return packetHelper.buildPacket(packetIDs.server_targetSilenced, (
        (fro, dataTypes.STRING),
        ('', dataTypes.STRING),
        (to, dataTypes.STRING),
        (fro_id or userUtils.getID(fro), dataTypes.SINT32)
...
channelInfoEnd = packetHelper.buildPacket(packetIDs.server_channelInfoEnd, (
    (0, dataTypes.UINT32),
))
...
def fellowSpectatorJoined(userID: int) -> bytes:
    return packetHelper.buildPacket(packetIDs.server_fellowSpectatorJoined, (
        (userID, dataTypes.SINT32),
    ))
...
def matchJoinSuccess(matchID: int) -> bytes:
    ...
    # Get match binary data and build packet
    match = glob.matches.matches[matchID]
    return packetHelper.buildPacket(packetIDs.server_matchJoinSuccess, match.getMatchData())
...
def rtx(message: str) -> bytes:
    return packetHelper.buildPacket(packetIDs.server_sendRTX, (
        (message, dataTypes.STRING),
    ))

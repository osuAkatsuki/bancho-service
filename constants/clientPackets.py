...
MATCH_SETTINGS_FMT_THIRD = (
    *[(f"slot{i}Mods", dataTypes.UINT32) for i in range(16)],
)
def matchSettings(stream):
    ...
    struct = [(f'slot{i}ID', dataTypes.SINT32) for i in range(16)
    ...
    data.update(packetHelper.readPacketData(stream[start:], MATCH_SETTINGS_FMT_THIRD, False)["data"])
